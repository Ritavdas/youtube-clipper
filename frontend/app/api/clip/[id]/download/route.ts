...
export async function GET(
	...
	try {
		...
		// Step 1: Get the job status from backend
		...
		// Step 2: Download the file directly from backend
		...
		// Step 3: Clean up the file via backend cleanup endpoint
		try {
			const cleanupRes = await fetch(
				`${backendUrl}/api/clip/${id}/cleanup`,
				{
					method: "DELETE",
				}
			);
