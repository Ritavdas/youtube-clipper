"use client";
// Auth client removed - no longer needed
...
// Buy component removed - no payment required
...
export default function Editor() {
	...
	// Premium state removed - no payment required
	...
	// Session removed - no authentication required
	// Download count removed - no user sessions
	// Premium modal removed - no payment required
	...
	// Download count tracking removed - no user sessions

	// Premium status check removed - no auth required

	const handleSubmit = async (e: React.FormEvent) => {
		e.preventDefault();

		// Premium check removed - always allow processing
	...
	return (
		<main className="flex flex-col w-full h-full min-h-screen p-4 gap-4 max-w-3xl mx-auto items-center justify-center">
			{/* Navigation removed - no user authentication */}
								...
								{Object.entries(resolutionOptions).map(
									([key, { icon, label }]) => (
										<div
											key={key}
											onClick={() =>
												setCropRatio(key as typeof cropRatio)
											}
